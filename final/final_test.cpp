...
class SDN_controller: public node {
        ...
        class Node {
            public:
                vector<unsigned int> new_weight_list; 
        ...
        unsigned int node_stat_cnt = 0; // counter to check if all the switches has sened stat packet to controller
        vector< pair<unsigned int, unsigned int> > old_path;
        vector< pair<unsigned int, unsigned int> > new_path;

        void setMatrixSize(); // preallocate memory for adjacency matrix 
        void Dijkstra(vector<Node> &node_list, unsigned int dst); // shortest path algorithm
        unsigned int getMin(vector<unsigned long> distance_arr, vector<bool> is_found); // function derived from Dijkstra()
        void setUPStatus(); // create the ctrl packet event at update time
        bool isRoundEnd();
        void simplifyGraph();
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet") {
        ...
        p->getHeader()->setNexID ( table[p->getHeader()->getDstID()] );
        send_handler (p);
    } else if (p->type() == "SDN_ctrl_new_packet" || p->type() == "SDN_ctrl_upd_packet") { // the switch receives a packet from the controller
        unsigned int mat;
        unsigned int act;
        if(p->type() == "SDN_ctrl_new_packet") {
            SDN_ctrl_new_payload *payload_ptr = dynamic_cast<SDN_ctrl_new_payload*> (dynamic_cast<SDN_ctrl_new_packet*>(p)->getPayload());
            mat = payload_ptr->getMatID();
            act = payload_ptr->getActID();
        } else {
            SDN_ctrl_upd_payload *payload_ptr = dynamic_cast<SDN_ctrl_upd_payload*> (dynamic_cast<SDN_ctrl_upd_packet*>(p)->getPayload());
            mat = payload_ptr->getMatID();
            act = payload_ptr->getActID();
        }
        table[mat] = act;

        unsigned int pre_id = p->getHeader()->getPreID();
        p->getHeader()->setSrcID ( getNodeID() );
        p->getHeader()->setDstID ( pre_id );
        p->getHeader()->setPreID ( getNodeID() );
        p->getHeader()->setNexID ( pre_id);
        send_handler (p);
    } else if(p->type() == "SDN_stat_packet") {
        unsigned int nxt_id = p->getHeader()->getDstID();
        p->getHeader()->setNexID(nxt_id);
        p->getHeader()->setPreID(node::getNodeID());
        send_handler(p);
    } else if(p->type() == "SDN_ctrl_del_packet") {
        SDN_ctrl_del_packet *pkt_ptr = dynamic_cast<SDN_ctrl_del_packet*> (p);
        SDN_ctrl_del_payload *payload_ptr = dynamic_cast<SDN_ctrl_del_payload*> (pkt_ptr->getPayload());

        unsigned mat = payload_ptr->getMatID();
        table.erase(mat);
...
class Data_handler {
    ...
    private:
        class Node;
    public:
        // single node that only store the neighbor and weights around itself (doesn't include the routing table)
        class Node_stat {
            public:
                vector<unsigned int> new_weight_list;
                Node_stat() {};
        ...
        vector<Node_stat> node_stat_list;
    ...
    public:
        ...
        void setStatEvent(); // create stat packet event in the beginning
        map<unsigned int, unsigned int> matrixToMap(const Data_handler::Node_stat node_stat); // convert information in matrix to a map
        void setCtrlPacket(); // SDN_ctrl_new_packet to install the routing table on switches (send to the controller first)

        // accessors
        unsigned int get_switch_num() { return switch_num; };
        unsigned int get_inst_time() { return inst_time; };
        unsigned int get_updt_time() { return updt_time; };
    ...
    private:
        unsigned int switch_num, node_cost;
        unsigned int inst_time, updt_time, dur_time;
        unsigned int src_id, dst_id, old_path_len;

        unsigned int old_link_id, cur_node, nxt_node;
        vector< pair<unsigned int, unsigned int> > path;

        unsigned int link_num;
        unsigned int link_id, node1, node2, new_weight;

        unsigned int event_time, event_src_id, event_dst_id;
};
Data_handler handler;

void Data_handler::startSetup() {
    // read number of node, dst, link and the network update time
    cin >> switch_num >> node_cost;
    cin >> inst_time >> updt_time >> dur_time;
    cin >> src_id >> dst_id >> old_path_len;

    path.reserve(old_path_len);
    for(unsigned int i = 0; i < old_path_len; i++) {
        cin >> old_link_id >> cur_node >> nxt_node;
        path.push_back(make_pair(cur_node, nxt_node));
    }
    ...
    for(unsigned int i = 0; i < link_num; i++) {
        cin >> link_id >> node1 >> node2 >> new_weight;
        node_stat_list.at(node1).new_weight_list.at(node2) = new_weight;
        node_stat_list.at(node2).new_weight_list.at(node1) = new_weight;
    ...
    cin >> event_time >> event_src_id >> event_dst_id;
    data_packet_event(event_src_id, event_dst_id, event_time);
...
void Data_handler::SetSize() {
    node_stat_list.resize(switch_num);
    for(auto &i : node_stat_list)
        i.new_weight_list.resize(switch_num, MAX);
}

void Data_handler::generateNode() {
    // generate switch nodes
    for (unsigned int id = 0; id < switch_num; id++)
...
void Data_handler::setStatEvent()
{
    unsigned int con_id = switch_num;

    for(unsigned int i = 0; i < switch_num; i++) {
        stat_packet_event(i, con_id, matrixToMap(node_stat_list.at(i)), updt_time, "new stat");
    }
}

map<unsigned int, unsigned int> Data_handler::matrixToMap(const Data_handler::Node_stat node_stat)
{
    map<unsigned int, unsigned int> stat_info;
    vector<unsigned int> weight_list;

    weight_list = node_stat.new_weight_list;

    for(unsigned int i = 0; i < switch_num; i++) {
        if(MAX != weight_list.at(i))
            stat_info[i] = weight_list.at(i);
    }
    
    return stat_info;
};

void Data_handler::setCtrlPacket()
{   
    unsigned int con_id = node::getNodeNum() - 1;

    for(auto i : path)
        ctrl_new_packet_event(con_id, i.first, dst_id, i.second, inst_time, "install");
}
...
void SDN_controller::recv_handler (packet *p) {
    ...
    unsigned int switch_num = node::getNodeNum() - 1;
    unsigned int con_id = switch_num;

    // resize the vectors when access for the first time
    if(!isInitialized) {
        setMatrixSize();
        old_path.reserve(switch_num);
        new_path.reserve(switch_num);
    ...
    if(p->type() == "SDN_ctrl_new_packet" && p->getPayload()->getMsg() == "install") {
        if(p->getHeader()->getPreID() != node::getNodeID())
            return;

        SDN_ctrl_new_packet *packet_ptr = dynamic_cast<SDN_ctrl_new_packet*> (p);
        SDN_ctrl_new_payload *payload_ptr = dynamic_cast<SDN_ctrl_new_payload*> (packet_ptr->getPayload());
        unsigned int node_id = p->getHeader()->getDstID();
        unsigned int act = payload_ptr->getActID();
        // old_path.push_back(make_pair(node_id, act));
    ...
    if(p->type() == "SDN_ctrl_upd_packet" || p->type() == "SDN_ctrl_new_packet" || p->type() == "SDN_ctrl_del_packet") {
        unsigned int nxt_id = p->getHeader()->getDstID();
        ...
        // ack_arr.at(p->getHeader()->getPreID()) = 2;

        // if(!isRoundEnd())
        //     return;

        // unsigned int dst_id = handler.get_dst_id(); 
        // for(unsigned int i = 0; i < status.size(); i++) {
        //     if(status.at(i) == 2 && ack_arr.at(i) == 0) {
        ...
        // // start update ctrl_upd_packet (first round
        // for(int i = simplify_order_path.size() - 2; i >= 0; i--) {
        //     unsigned node_id = simplify_order_path.at(i);
        //     if(status.at(node_id) == 1 && ack_arr.at(node_id) == 0) {
        //         ctrl_upd_packet_event(con_id, node_id, dst_id, node_list.at(node_id).new_table.at(dst_id));
        //         ack_arr.at(node_id) = 1;
        //         return;
        //     }
        // }
        
        // // if there is no upd_packet to generate
        // for(int i = 0; i < status.size(); i++) {
        //     if(status.at(i) == 0 && ack_arr.at(i) == 0) {
        //         ctrl_del_packet_event(con_id, i, dst_id, node_list.at(i).new_table.at(dst_id));
        //         ack_arr.at(i) = 1;
        //         return;
        //     }
        // }
    ...
    if(p->type() == "SDN_stat_packet") {
        SDN_stat_payload *payload_ptr = dynamic_cast<SDN_stat_payload*>(dynamic_cast<SDN_stat_packet *>(p)->getPayload());
        map<unsigned int, unsigned int> stat_info = payload_ptr->getLinkWeight();

        unsigned int node_id = p->getHeader()->getPreID(); // get the sender of packet
        for(auto i : stat_info)
            node_list.at(node_id).new_weight_list.at(i.first) = i.second;

        node_stat_cnt++;
        if(node_stat_cnt == node::getNodeNum() - 1) {
            Dijkstra(node_list, handler.get_dst_id());
            unsigned cur_node = handler.get_src_id();
            while(cur_node != handler.get_dst_id()) {
...
void SDN_controller::setMatrixSize() {
    unsigned int node_num = node::getNodeNum() - 1;
...
void SDN_controller::Dijkstra(vector<Node> &node_list, unsigned int dst)
{   
...
unsigned int SDN_controller::getMin(vector<unsigned long> distance_arr, vector<bool> is_found)
{
...
void SDN_controller::setUPStatus() {
    unsigned int dst_id = handler.get_dst_id();
    unsigned int switch_num = node::getNodeNum() - 1;
    unsigned con_id = switch_num;

    status.resize(switch_num, 0);

    for(unsigned int i = 0; i < new_path.size(); i++) {
        unsigned int node1 = new_path.at(i).first;
        unsigned int node2 = new_path.at(i).second;

        unsigned int j;
        for(j = 0; j < old_path.size(); j++) {
            if(old_path.at(j).first == node1) {
                if(old_path.at(j).second != node2) {
                    status.at(new_path.at(i).first) = 1;
                } else {
                    status.at(new_path.at(i).first) = 3;
                }
                break;
            }
        }
        if(j == old_path.size())
            status.at(new_path.at(i).first) = 2;
    }
    
    for(unsigned int i = 0; i < old_path.size(); i++) {
        if(status.at(old_path.at(i).first) == -1)
            status.at(old_path.at(i).first) = 0;
    }

    // simplify the graph into a vector of integers
    // simplifyGraph();
...
>>>>>>> 9c3ac1a401e59a250d0a7bc32ee557769af62825
    ...
    //     // start update ctrl_upd_packet (first round)
    //     for(int i = simplify_order_path.size() - 2; i >= 0; i--) {
    //         if(status.at(simplify_order_path.at(i)) == 1 && ack_arr.at(simplify_order_path.at(i)) == 0) {
    //             ctrl_upd_packet_event(con_id, simplify_order_path.at(i), dst_id, node_list.at(simplify_order_path.at(i)).new_table.at(dst_id));
    //             ack_arr.at(simplify_order_path.at(i)) = 1;
    //             return;
    //         }
    //     }
    // }
...
bool SDN_controller::isRoundEnd() {

    for(unsigned int i = 0; i < ack_arr.size(); i++) {
        if(i != handler.get_dst_id() && ack_arr.at(i) == 1)
            return false;
    }
    return true;
}

void SDN_controller::simplifyGraph()
{
    for(unsigned int i = 0; i < new_path.size(); i++) {
        if(i == 0)
            simplify_order_path.push_back(new_path.at(i).first);
        simplify_order_path.push_back(new_path.at(i).second);  
    }

    bool is_simplified = false;
    while(!is_simplified) {
        is_simplified = true;
        for(unsigned int i = 0; i < simplify_order_path.size() - 1; i++) {
            if(status.at(simplify_order_path.at(i + 1)) == 0 || status.at(simplify_order_path.at(i + 1)) == 2) {
                simplify_order_path.erase(simplify_order_path.begin() + i + 1);
                is_simplified = false;
            }
        }
    }
...
>>>>>>> 9c3ac1a401e59a250d0a7bc32ee557769af62825
    ...
    // freopen(".\\test6.txt","r",stdin);
    // freopen(".\\output_test6.txt","w",stdout);
