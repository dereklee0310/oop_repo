...
class SDN_stat_pkt_gen_event: public event {
    ...
    private:
        ...
        // unsigned int act;
        // packet *pkt; // the packet
        vector< pair<unsigned int, unsigned int> > stat_info; //? DONE?
        string msg;
    ...
    protected:
        SDN_stat_pkt_gen_event (unsigned int _trigger_time, void *data): event(_trigger_time), src(BROCAST_ID), dst(BROCAST_ID){
            ...
            // mat = data_ptr->mat_id;
            // act = data_ptr->act_id;
            stat_info = data_ptr->stat_info; //? DONE?
            msg = data_ptr->msg;
        } 
...
// TODO: change the hash value
unsigned int SDN_stat_pkt_gen_event::event_priority() const {
    string string_for_hash;
    //? DONE?
    string_for_hash = to_string(getTriggerTime()) + to_string(src) + to_string(dst) + to_string(rand()); //to_string (pkt->getPacketID());
    return get_hash_value(string_for_hash);
}
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet") {
        ...
        unsigned int nex_id = p4->getHeader()->getDstID(); // destination is the controller
...
class SDN_controller: public node {
    ...
    public:
        ...
        void setCtrlEvent(bool is_old = true); // create the ctrl packet event in the beginning
...
class Data_handler {
    ...
    public:
        ...
        unsigned int get_node_num() { return node_num; }; // accesssor
        unsigned int get_dst_num() { return dst_num; }; // accesssor
        unsigned int get_link_num() { return link_num; }; // accesssor
        unsigned int get_dur_time() { return dur_time; }; // accesssor
        unsigned int get_inst_time() { return inst_time; }; // accesssor
        unsigned int get_updt_time() { return updt_time; }; // accesssor
        vector<unsigned int> get_dest_list() { return dest_list; }; // accesssor
...
void SDN_controller::recv_handler (packet *p) {
    ...
    if(p->type() == "SDN_stat_packet") {
        unsigned int node_id = p3->getHeader()->getPreID();
    ...
    for(unsigned int i = 0; i < node_num; i++) {
        ...
        for(unsigned int j = 0; j < handler.get_dst_num(); j++) {
            ...
            if(is_old) {
                ...
                ctrl_packet_event(con_id, node_id, cur_dst_id, next_id); 
            } else {
...
vector< pair<unsigned int, unsigned int> > getStatInfo(const Data_handler::Node_stat node_stat, bool &is_old_stat)
{
    ...
    if(is_old_stat) {
        ...
        is_old_stat = false; // for the next time using it
    } else {
        weight_list = node_stat.new_weight_list;
        is_old_stat = true; // for the next time using it
    }

    for(unsigned int i = 0; i < switch_num; i++) {
        if(MAX != weight_list.at(i))
            stat_info.push_back(make_pair(i, weight_list.at(i)));
    }
