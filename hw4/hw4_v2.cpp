...
/*****************************************
 * implementation of hw4 start from here *
 *****************************************/
...
class SDN_stat_payload : public payload {
        ...
        // *store the information about neighbors and weights around specific node
        vector< pair<unsigned int, unsigned int> > stat_info;
    ...
    public:
        ...
        // *mutator and accessor of stat_info
        void set_stat_info(vector< pair<unsigned int, unsigned int> > _stat_info) { stat_info = _stat_info; };
        vector< pair<unsigned int, unsigned int> > get_stat_info() { return stat_info; };
...
class SDN_stat_pkt_gen_event: public event {
    ...
    protected:
        SDN_stat_pkt_gen_event (unsigned int _trigger_time, void *data): event(_trigger_time), src(BROCAST_ID), dst(BROCAST_ID){
            ...
            src = data_ptr->src_id;
            dst = data_ptr->dst_id;
            // *setup the data of stat_pkt_event
            stat_info = data_ptr->stat_info;
            msg = data_ptr->msg;
        } 
...
void SDN_stat_pkt_gen_event::trigger() {
    ...
    /***************************************************
     * packet will be sended from switch to controller *
     ***************************************************/
    hdr->setSrcID(src); 
    hdr->setDstID(dst);
    hdr->setPreID(src);
    ...
    // *setup the payload of packet
    pld->setMsg(msg);
    pld->set_stat_info(stat_info);
...
unsigned int SDN_stat_pkt_gen_event::event_priority() const {
    string string_for_hash;
    string_for_hash = to_string(getTriggerTime()) + to_string(src) + to_string(dst) + to_string(rand()); // *use rand() number as a part of string for hashing
    return get_hash_value(string_for_hash);
}
...
/****************************************************
 * the implementation of recv_handler in SDN_switch *
 ****************************************************/
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet") {
        ...
        // *switch received packet, forward it to the controller
        unsigned int nex_id = p4->getHeader()->getDstID();
        p4->getHeader()->setNexID (nex_id);
        send_handler (p4);
    }
}
...
class Data_handler {
    ...
    public:
        ...
        unsigned int get_link_num() { return link_num; };
        ...
        vector<unsigned int> get_dest_list() { return dest_list; };
...
void Data_handler::startSetup() {
    ...
    for(unsigned int i = 0; i < link_num; i++) {
        ...
        node_stat_list.at(node1).old_weight_list.at(node2) = old_weight;
        node_stat_list.at(node2).old_weight_list.at(node1) = old_weight;
...
void Data_handler::SetSize() {
    ...
    //* new code
    node_stat_list.resize(node_num);
...
// *function that convert adjacency matrix to vector of pairs (neighbor, weight)
vector< pair<unsigned int, unsigned int> > getStatInfo(const Data_handler::Node_stat node_stat, bool &is_old_stat);
// *the stat_packet_event function is used to add an initial event
void stat_packet_event (unsigned int con_id, unsigned int id, vector< pair<unsigned int, unsigned int> > stat_info, unsigned int t = event::getCurTime(), string msg = "default");

void Data_handler::setStatEvent()
{
    unsigned int switch_num = node::getNodeNum() - 1;
    unsigned int con_id = switch_num;
    bool is_old_stat = true;

    for(unsigned int i = 0; i < switch_num; i++) {
        stat_packet_event(con_id, i, getStatInfo(node_stat_list.at(i), is_old_stat), inst_time, "old stat");
        stat_packet_event(con_id, i, getStatInfo(node_stat_list.at(i), is_old_stat), updt_time, "new stat");
    }
}

void stat_packet_event (unsigned int con_id, unsigned int id, vector< pair<unsigned int, unsigned int> > stat_info, unsigned int t, string msg) // t = event::getCurTime(), string msg = "default"
{
// void stat_packet_event (unsigned int dst, unsigned int t = 0, string msg = "default"){
    ...
    // send it from switch to controller
    unsigned int src = id;
    e_data.src_id = src;
    e_data.dst_id = con_id;
    e_data.stat_info = stat_info;
    e_data.msg = msg;
...
vector< pair<unsigned int, unsigned int> > getStatInfo(const Data_handler::Node_stat node_stat, bool &is_old_stat)
{
    ...
    if(is_old_stat) {
        weight_list = node_stat.old_weight_list;
        is_old_stat = false;
    } else {
    ...
    for(unsigned int i = 0; i < switch_num; i++) {
        if(MAX != weight_list.at(i))
            stat_info.push_back(make_pair(i, weight_list.at(i))); //* first: neighbor id, second: weight
    }
...
class SDN_controller: public node {
    ...
    public:
        ...
        /************************************************
         * member and member function that added in hw3 *
         ************************************************/
        ...
        /************************************************
         * member and member function that added in hw4 *
         ************************************************/
        ...
        bool is_old = true; // *flag that decide whether to install or update
        ...
        void buildTable(bool is_old = true); // build the old or new routing table
        ...
        void setCtrlEvent(bool is_old = true); // *create the ctrl packet event at install and update time
...
void SDN_controller::recv_handler (packet *p) {
    ...
    if(p->type() == "SDN_ctrl_packet") {
        ...
        // *casting packet and payload to SDN_stat
    ...
    if(p->type() == "SDN_stat_packet") {
        unsigned int node_id = p3->getHeader()->getPreID(); //* get the sender of packet
        vector< pair<unsigned int, unsigned int> > stat_info = l3->get_stat_info();
        for(auto i : stat_info) {
            if(is_old)
                node_list.at(node_id).old_weight_list.at(i.first) = i.second;
            else
                node_list.at(node_id).new_weight_list.at(i.first) = i.second;
        }

        node_stat_cnt++;
        if(node_stat_cnt == switch_num) {
            buildTable(is_old);
            setCtrlEvent(is_old);
            is_old = false; // change state to new
            node_stat_cnt = 0; // reset the counter
        }
        return;
    }
...
void SDN_controller::setMatrixSize() {
    ...
    for(auto &i : node_list) {
        i.old_weight_list.resize(node_num, MAX);
        i.new_weight_list.resize(node_num, MAX);
    }
}

void SDN_controller::buildTable(bool is_old) {
    unsigned int node_num = node::getNodeNum() - 1;
    for(unsigned int i = 0; i < handler.get_dst_num(); i++) {
        unsigned int dest_id = handler.get_dest_list().at(i);
        Dijkstra(node_list, dest_id, is_old);
    }
}

void SDN_controller::Dijkstra(vector<Node> &node_list, unsigned int dst, bool is_old)
{   
...
void SDN_controller::setCtrlEvent(bool is_old) {
    ...
    for(unsigned int i = 0; i < node_num; i++) {
        ...
        for(unsigned int j = 0; j < handler.get_dst_num(); j++) {
            // don't create a packet if the destination of node is itself 
            unsigned int cur_dst_id = handler.get_dest_list().at(j);
            if(node_id == cur_dst_id)
                continue;

            if(is_old) {
                next_id = node_list.at(i).old_table.at(cur_dst_id);
                ctrl_packet_event(con_id, node_id, cur_dst_id, next_id, event::getCurTime(), "install"); 
            } else {
                next_id = node_list.at(i).new_table.at(cur_dst_id);
                ctrl_packet_event(con_id, node_id, cur_dst_id, next_id, event::getCurTime(), "update");
            }
        }
    }
}
...
{       
     ...
     ******************************************/
    // freopen(".\\samples-HW4\\1.in","r", stdin);
    // freopen(".\\output1.txt","w", stdout);

    /******************************************
     * reassign the file pointers: test case2 *
     ******************************************/
    // freopen(".\\samples-HW4\\2.in","r",stdin);
